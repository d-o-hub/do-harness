...
set edit:completion:arg-completer[do-harness] = {|@words|
    ...
    var completions = [
        ...
        &'do-harness;verify'= {
            ...
            cand --task 'Scope records and fail-fast strikes to this task id (or ''global'')'
            ...
            cand --global 'Record unscoped beats in the global namespace'
        ...
        &'do-harness;metrics'= {
            ...
            cand --scope 'Filter by workstream scope (e.g. `branch:main`, `task:1`, `global`, or `all`). Defaults to the current git branch'
            cand --task 'Filter by task id (equivalent to `--scope task:<ID>`)'
            cand --branch 'Filter by branch name (equivalent to `--scope branch:<NAME>`)'
            cand --root 'Workspace root override (default: walk up from cwd)'
            cand --config 'Explicit path to do-harness.toml'
            cand --color 'Color output (auto, always, never)'
            cand --output 'Default output file path'
            cand --all 'Aggregate across all workstreams and scopes'
            cand -v 'Verbosity level (-v, -vv)'
            cand --verbose 'Verbosity level (-v, -vv)'
            cand -q 'Suppress non-error messages'
            cand --quiet 'Suppress non-error messages'
