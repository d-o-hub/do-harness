...
- **Sensor policy decisions**: explicit wontfix entries (gitleaks, SBOM, lockfile freshness, eval bless-drift) live in `plans/invariants.json`; `do-harness seed --prune` keeps libSQL in sync and drops stale rows. `shell` (shellcheck, fail-closed in CI), `check-agt` (guardian-proxy governance feature), and `check-promotions` (a recorded `hold`/`promote` must match the shipped Cargo defaults — hermetic, because the GA watch itself exits 0 on every verdict and needs the network) are workspace sensors.
