...
- **PR lifecycle**: after opening a PR, load `.agents/skills/pr-triage` and run the sweep without waiting to be asked (review/roast the diff, resolve threads, wait for every check, merge with `gh pr merge --squash --match-head-commit <validated head>`; never auto-merge). In a dev checkout set `DO_HARNESS_BIN=target/release/do-harness` so the skill uses `do-harness pr review` instead of falling back to the full diff.
- **Persistence commands**: `do-harness task list`/`task export` surface task state (libSQL is the source of truth; export writes `plans/tasks.json` with a board summary and `task import --check` validates freshness — import is validation-only by design, since ids are database-assigned and the event log is append-only, so a snapshot cannot be replayed back into the database); `verify --record` persists beats (scoped to the current git branch by default, to `--task <id>` when passed, or `--global`), and bumps error signatures for failing sensors; per-sensor `severity = "error"|"warn"` (with `allow_failure` as the deprecated warn alias) makes warn failures advisory except under `--strict` outside `feedback`, and warn strikes quarantine after 3; `FINDINGS: <n>` markers ratchet against the committed `plans/baselines.json`, which `verify --record --bless` only lowers (raise refused, bless history in libSQL); `task add/advance/fail`, `trace add/list`, `distill`, `eval`, `metrics`, and `maintenance --prune-beats` are implemented. `plans/methods.json` ships `vertical-event-slice`, `spike-and-resolve`, and `decision` (a promotion verdict gated on the decision-agnostic `check-promotions`). The global `--dry-run` covers every mutating `task` action including `add` and `export`; `task remove` refuses a task that has workflow events rather than deleting hash-chained audit rows.
...
- **LOC feedforward**: the 500-LOC ceiling is enforced by the `loc` sensor (`scripts/check-loc.sh`), which on failure prints the largest top-level item spans (`SPAN:`) and the code/test split (`CODE:`/`TEST:`) and always reports a `FINDINGS: <n>` over-limit count for the blessed ratchet. `--root`/`--ext` widen the scan beyond the default `*.rs` under `src/`+`crates/` (generated trees are pruned), so a front-end tree gets the same ceiling from `do-harness.toml` without forking the script. `do-harness loc [--warn] [--format json] [paths…]` answers the same question without running the sensor, and `do-harness split <file> [--dry-run] [--target <name>]` performs the decomposition: it extracts the inline `#[cfg(test)]` module to `<stem>_tests.rs` (declared with `#[path = …]`), or the largest top-level item that both fits the ceiling and brings the source under it, and refuses with a diagnostic rather than emitting code that cannot compile (`macro_rules!`, `include!`, non-test `#[cfg]`).
- **Sensor policy decisions**: explicit wontfix entries (gitleaks, SBOM, lockfile freshness, eval bless-drift) live in `plans/invariants.json`; `do-harness seed --prune` keeps libSQL in sync and drops stale rows. `shell` (shellcheck, fail-closed in CI), `check-agt` (guardian-proxy governance feature), `powerset` (guardian-proxy feature-powerset check), `machete` (unused-dependency pruning), `untrusted-ingest` (the `pr-triage` skill is the only component whose input is attacker-controlled by design: its ingest scripts must hold no dynamic-execution sink — `eval`, `sh -c`, `child_process.exec`, shell-enabled `spawn` — and its untrusted-data contract clause must still be stated; a sink lint, not a taint tracker, so it proves the dangerous sinks are absent rather than exhaustively proving no path exists), and `check-promotions` (every recorded `*-promotion` verdict must agree with the shipped Cargo defaults — a coherence gate only; it does not re-derive GA, because the GA watch exits 0 on every verdict and needs the network, so the dated verdict stays recorded epic evidence) are workspace sensors.
- **Artifact Provenance Contract**: language-neutral verification of release-artifact provenance (e.g., `artifact-provenance` sensor). Required inputs: artifact path(s) (`ARTIFACT_PATHS`), expected digest (`EXPECTED_DIGEST`), and verification mode (`VERIFY_MODE`). Evidence output must emit a `COVERAGE: <json>` record containing `artifact`, `digest`, `status` (`pass`|`fail`), and `reason` on failure. Only `digest-only` is implemented: reserved modes (`slsa`, `github-attestation`, `sbom`, `strict`) fail closed rather than reporting a pass they did not verify.
