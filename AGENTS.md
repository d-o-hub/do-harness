...
- **Workflow gates**: pre-commit = `do-harness verify --fail-fast --only fmt --only loc`; pre-push = `do-harness verify --fail-fast`; CI = `verify.yml` lints shell + `cargo check --features agt-governance` + `do-harness verify --format json --evidence .do-harness/evidence.json --strict` (exit 0/1/2) + dogfood `init && verify` + `do-harness eval`.
