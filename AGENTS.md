...
- **Persistence & Roadmap**: the do-harness-db repo layer implements persistence writers for tasks/beats/traces/heuristics/error_signatures/skill_evals (`verify --record` persists beats scoped to `--task`, resetting a sensor's strikes on pass; `task list`/`task export` read task state). The workflow runtime (`task add/advance/done/fail`, gated on the `plans/methods.json` catalog and sensor beats), fail-fast recovery (`errors list/clear`, per-task strike counters), the skill-eval runner (`do-harness eval` grades deterministic artifact assertions with an optional walkthrough, gated by skill-creator's quick_validate.py), the distill CLI (`distill` writes into the skill corpus + `trace add/list`), and commitlint enforcement (`scripts/check-commitlint.sh`, the `commitlint` sensor) are all implemented. No roadmap items remain (see plans/invariants.json "Conventional commits" invariant).
