...
- **Artifact Provenance Contract**: language-neutral verification of release-artifact provenance (e.g., `artifact-provenance` sensor). Required inputs: artifact path(s) (`ARTIFACT_PATHS`), expected digest (`EXPECTED_DIGEST`), and verification mode (`VERIFY_MODE`). Evidence output must emit a `COVERAGE: <json>` record containing `artifact`, `digest`, `status` (`pass`|`fail`), and `reason` on failure.
