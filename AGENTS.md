...
- **Sensor policy decisions**: explicit wontfix entries (gitleaks, SBOM, lockfile freshness, eval bless-drift) live in `plans/invariants.json`; `do-harness seed --prune` keeps libSQL in sync and drops stale rows. `shell` (shellcheck, fail-closed in CI), `check-agt` (guardian-proxy governance feature), and `check-promotions` (every recorded `*-promotion` verdict must agree with the shipped Cargo defaults — a coherence gate only; it does not re-derive GA, because the GA watch exits 0 on every verdict and needs the network, so the dated verdict stays recorded epic evidence) are workspace sensors.
