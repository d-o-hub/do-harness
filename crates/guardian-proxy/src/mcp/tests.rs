...
fn test_mediator(upstream: &str) -> Arc<ProxyMediator> {
    test_mediator_with_origins(upstream, crate::default_allowed_origins())
}

fn test_mediator_with_origins(upstream: &str, allowed_origins: Vec<String>) -> Arc<ProxyMediator> {
...
#[tokio::test(flavor = "current_thread")]
async fn denied_call_maps_to_tool_error_with_audit() {
    use rmcp::model::CallToolResponse;

    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("audit.jsonl");
    let audit = AuditLog::open(&path).expect("audit");
    let state = crate::AppState::with_audit(test_mediator("http://127.0.0.1:9"), audit);
    let metrics = std::sync::Arc::clone(&state.metrics);
    let ingress = super::McpIngress::new(state);

    // The off-feature stub denies non-object params; the typed MCP request
    // shape cannot express that, so exercise mediation directly.
    let call = crate::McpLikeToolCall::new("echo", Some(json!("not an object")));
    let denial = ingress
        .mediate(&call)
        .await
        .expect("mediate")
        .expect("denied");
    let CallToolResponse::Complete(result) = denial else {
        panic!("expected a complete tool result");
    };
    assert_eq!(result.is_error, Some(true));
    assert!(matches!(
        result.content.first(),
        Some(rmcp::model::ContentBlock::Text(text))
            if text.text.contains("denied by governance")
    ));
    ...
    AuditLog::verify(&path).expect("intact chain");
    let contents = std::fs::read_to_string(&path).expect("audit file");
    assert!(contents.contains(r#""decision":"deny""#), "{contents}");
}
