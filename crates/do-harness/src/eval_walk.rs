...
/// inside the retry budget so the post-fix run cannot time out.
#[cfg(all(test, unix))]
...
fn spawn_walkthrough(script: &Path, root: &Path, bin: &Path) -> std::io::Result<Output> {
    // The retry covers the `ETXTBSY` fork/exec race: the script was just
    // written, so an exec can fail while another thread is mid-write or
    // mid-spawn. See [`crate::shell::retry_executable_busy`].
    let attempt = crate::shell::retry_executable_busy(|| run_script(script, root, bin));
    match attempt {
        Ok(output) => Ok(output),
