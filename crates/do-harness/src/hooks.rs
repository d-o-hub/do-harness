...
if [[ ! -x "$BIN" ]]; then
  echo "do-harness: missing $BIN (build with: cargo build --release -p do-harness)" >&2
  exit 2
fi
...
pub struct HookStatus {
    ...
    pub pre_push: bool,
    /// Whether `<repo_root>/target/release/do-harness` exists.
...
pub fn status(git_dir: &Path, repo_root: &Path) -> HookStatus {
    HookStatus {
        ...
        binary_exists: repo_root.join("target/release/do-harness").is_file(),
    }
}
...
fn script_body(only_args: &str) -> String {
...
fn only_args(sensors: &[String]) -> String {
